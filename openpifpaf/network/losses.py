...
def laplace_loss(x1, x2, logb, t1, t2, weight=None):
    ...
    二维向量(x1, x2)的loss, 真实值为(t1,t2), 半径为b
    Loss for a single two-dimensional vector (x1, x2) with radial
    spread b and true (t1, t2) vector.
    """
    norm = torch.sqrt((x1 - t1)**2 + (x2 - t2)**2)
    # 这里的losses对应公式（2），但是应用的L2 loss。
    # 由ln2 = 0.693可知，这个log指的是ln
    # torch.exp(-logb) = 1/b,因为传入的是logb,所以只得这么表示
...
class SmoothL1Loss(object):
    ...
    def __call__(self, x1, x2, _, t1, t2, weight=None):
        ...
        r = self.r_smooth * self.scale  # 半径
        d = torch.sqrt((x1 - t1)**2 + (x2 - t2)**2)
        smooth_regime = d < r

        # smooth_loss = L2loss^2/(2*r_smooth*scale)
        smooth_loss = 0.5 / r[smooth_regime] * d[smooth_regime] ** 2  # 这里注意d可能会是一个数,torch中可以这么做
