...
def factory(*,
            ...
            dilation=None,
            dilation_end=None):
    # 如果没有 checkpoint 并且传入了一个 basenet (basenet 的 default 为 none)
...
# pylint: disable=too-many-branches
# basename 为 backbone 的模型,例如 resnet50block5，无默认值。
# headnames 默认为 ['pif', 'paf']
def factory_from_scratch(basename, headnames, *, pretrained=True):
    ...
    # all blocks
    blocks = [
        resnet_factory.input_block(use_pool, conv_stride, pool_stride),  # torch.nn.Sequential
    ...
    def create_headnet(name, n_features):  # pylint: disable=too-many-return-statements
        for head in heads.Head.__subclasses__():  # Head 的 subclass 有 CompositeField
            logging.debug('checking whether head %s matches %s',
                          head.__name__, name)
            if not head.match(name):  # 默认的话会 match 到 pad 与 paf
                continue
